...
class ViewSlider{
    ...
    getSliderThumbs(): void{
        const ViewThumbs = new ViewThumbsContainer(
            '.my-slider-custom-app__view',
            'view__thumbs-container')
        ViewThumbs.getViewThumbsContainer()
        const ViewThumbContainerMin = new ViewThumbContainer(
            '.view__thumbs-container',
            'view__thumb', 'min' )
        const ViewThumbContainerMax = new ViewThumbContainer(
            '.view__thumbs-container',
            'view__thumb',
            'max' )
        ViewThumbContainerMin.getViewThumbContainer()
        ViewThumbContainerMax.getViewThumbContainer()
        const viewThumbMin = new ViewThumb(
            '.my-slider-custom-app__view',
            'topThumb', 'view__thumb',
            'bottomThumb',
            '.my-slider-custom-app__view',
            'min' )
        viewThumbMin.getViewThumb()
        viewThumbMin.handle_thumb()
        const viewThumbMax = new ViewThumb(
            '.my-slider-custom-app__view',
            'topThumb',
            'view__thumb',
            'bottomThumb',
    ...
    getSliderMaxMinValues(): void {
        ...
        new ViewValuesMin(
            '.view__max-min-container',
            'view__min-container').getViewValueMin()
        new ViewValuesMax(
            '.view__max-min-container',
            'view__max-container').getViewValueMax()
    }

    getSliderScales(): void {
        new ViewScaleContainer(
            '.my-slider-custom-app__view',
            'view__scale-container',
            'view__thumb',
            {max: "max", min: 'min'}).getViewScaleContainer()
        new ViewScaleContainer(
            '.my-slider-custom-app__view',
            'view__scale-container',
            'view__thumb',
            {max: "max", min: 'min'}).onScaleClick()
        new ViewScaleBackLine(
            '.view__scale-container',
            'view__scale-backLine').getViewScaleBackLine()
        new ViewScaleTrack(
            '.view__scale-container',
            'view__scale-track').getViewScaleTrack()
    }
    getViewRuler(): void {
        new ViewRulerContainer(
            '.my-slider-custom-app__view',
            'view__ruler-container').getViewRulerContainer()
        new ViewRuler(
            '.view__ruler-container',
            'view__ruler').observeRulerSize()
    }
